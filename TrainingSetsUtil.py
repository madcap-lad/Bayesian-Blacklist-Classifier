"""
Created on Tue May 08 21:58:45 2018

handles operations involving tokenizing, counting words, etc.
...
blockedfilepath = 'data/logs/blocked'
nonBlockedfilepath = 'data/logs/non-block'
...
def make_training_set2(path):
    ...
    cmds_count = 1
    # total number of files in the directory
    total_file_count = 1    #len(mails_in_dir)
    ...
    #for mail_name in mails_in_dir:
        ...
        # get the message in the mail
    message = get_mail_from_file(path)
    ...
    # reducing the count of cmds files from file count
    total_file_count = file_len(path);
    # calculating the occurrence for each term
    for term in training_set.keys():
      training_set[term] = float(training_set[term]) / total_file_count
...
spam_training_set = make_training_set2(blockedfilepath)
ham_training_set = make_training_set2(nonBlockedfilepath)
print('done.')
print('')